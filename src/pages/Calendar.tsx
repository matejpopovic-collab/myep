  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="Every known event, whether or not a WOF exists for it. Status comes from the WOF, so this view and the pipeline can never disagree."
        actions={
          <>
      ...
      {view === 'month' ? (
        <MonthView rows={rows} month={month} onRaise={openRaise} onDay={(day, events) => setDay({ day, events })} />
      ) : (
        <ListView rows={rows} onRaise={openRaise} />
      )}
...
function ListView({ rows, onRaise }: { rows: W.CalendarRow[]; onRaise: (id: string | null) => void }) {
  if (!rows.length) {
    return (
      <div className="card">
        <EmptyState
  ...
  return (
    <>
      <DataTable
        ...
        rowHref={(r) => (r.wof ? `/wofs/${r.wof.id}` : `/schedules?id=${r.scheduleId}`)}
        footer={{
